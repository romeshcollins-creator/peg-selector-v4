...
def recommendation_score_and_explanation(row):
    ...
    explanation = ", ".join(reasons) if reasons else "Matches partially or no filters selected"
...
# -----------------------------
# Display results in accordion (economical spacing)
# -----------------------------
...
for _, row in filtered_df.iterrows():
    ...
    with st.expander(f"{badge} {row['Product Name']}  |  Score: {row['Recommendation Score']}"):
        # Top PEG visual highlight
        if row["Recommendation Score"] == top_score:
            st.markdown(
                "<div style='background-color:#FFF3CD; padding:5px; border-radius:5px;'>"
                "<strong>🏆 Top Recommended PEG</strong></div>",
                unsafe_allow_html=True
            )

        # Concise explanation
        st.markdown(f"**Why recommended:** {row['Recommendation Explanation']}")
        st.markdown(f"**MW (kDa):** {row['Molecular Weight (kDa)']}  |  "
                    f"**FG/React:** {row['Functional Group / Reactivity']}  |  "
                    f"**Architecture:** {row['Polymer Architecture']}")
        st.markdown(f"**Application:** {row['Intended Application']}  |  "
                    f"**PDI:** {row['Polydispersity Index (PDI)']}  |  "
                    f"**Solubility:** {row['Solubility']}  |  "
                    f"**Partner:** {row['Commercial Partner']}")
